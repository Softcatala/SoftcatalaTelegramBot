...
class LangpackModule(object):
    ...
    def platform_handler(self, bot, update):
        ...
        with open('packs/stats.csv','a',newline='') as f:
             writer=csv.writer(f)
        ...
        bot.answerCallBackQuery(callback_query_id, text="")
    ...
    def stats_command(self, bot, update):
        ...
        # Replace USER_ID with your user_id number:
        if user_id == USER_ID:
            downloads = open('packs/stats.csv', 'r')
            total = len(downloads.readlines())
            downloads.close()
            f = open("packs/stats.csv", "r")
            downand = 0
            for line in f:
            ...
            stats_file= open('packs/stats.csv', 'rb')
            bot.sendDocument(update.message.chat_id,
                             document=stats_file) 
